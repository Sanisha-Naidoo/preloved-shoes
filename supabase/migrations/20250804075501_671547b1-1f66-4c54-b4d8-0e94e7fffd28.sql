-- Preloved Schema Security Fixes - Phase 1 (Drop Functions First)
-- Remove dangerous SQL execution function and implement security controls

-- 1. Drop the dangerous execute_sql function from preloved schema
DROP FUNCTION IF EXISTS preloved.execute_sql(text, text[]);

-- Drop existing functions that we'll recreate with proper signatures
...
-- Rate limiting table for IP-based submission control
...
-- 4. Fix existing functions in preloved schema with proper search_path and validation

-- Update create_shoe function with security controls
...
-- Update update_shoe_qr_code function with security controls
...
-- Update check_shoe_exists function with proper search_path (fixed reserved word)
...
-- Update get_shoe_count function with proper search_path
...
-- Update get_shoes_for_notion function with proper search_path
