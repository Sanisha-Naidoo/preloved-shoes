-- Clean up conflicting function definitions in preloved schema

-- Drop all existing versions of conflicting functions
DROP FUNCTION IF EXISTS preloved.create_shoe(text, text, text, text, text, integer, text, text);
DROP FUNCTION IF EXISTS preloved.create_shoe(text, text, text, text, text, text, text, text);
DROP FUNCTION IF EXISTS preloved.create_shoe(text, text, text, text, text, integer, text, uuid);

DROP FUNCTION IF EXISTS preloved.check_shoe_exists(text);
DROP FUNCTION IF EXISTS preloved.check_shoe_exists(uuid);

DROP FUNCTION IF EXISTS preloved.update_shoe_qr_code(uuid, text);

-- Create single, consistent versions of each function

-- Function to create a shoe record (required params first, then optional)
